...
# 启动SLAM
gnome-terminal -t "LIO" --working-directory="$LIO_WS_DIR"  -- bash -c  "source ./devel/setup.bash;roslaunch fast_lio mapping_unitree_G1.launch;exec bash;"
sleep 2s

# 启动OctoMapping
gnome-terminal -t "octo_mapping" --working-directory="$LIO_WS_DIR"  -- bash -c  "source ./devel/setup.bash;roslaunch fast_lio octo_mapping.launch;exec bash;"
sleep 2s
